...
public class RRT implements Tactic{
    ...
    public RRT(TacticMaker tacticMaker){
        this.tacticMaker = tacticMaker;

        this.start = new SearchNode(tacticMaker.getCurrentPosition().x, tacticMaker.getCurrentPosition().y);
        this.minRand = 0;
        this.maxRand = 1;
    ...
    public boolean checkAvailability(SearchNode resultNode, int[][] enemiesObservationGrid, PointSimulator.PointStatus pointStatus){
        for(int i=0;i<60;i++){
            for(int j=0;j<45;j++){
                ...
                if(        x>=0 && x<849
                        ...
                        || Systems.pointSimulator.isPointNotEmpty(x, y, pointStatus))){
                    return false;
                }
            }
        }
...
//        }
    ...
    @Override
    public void decide() {
        start = new SearchNode(tacticMaker.getCurrentPosition().x, tacticMaker.getCurrentPosition().y);
        this.nodeList.clear();
        ...
        for(int i = 0; i < this.maxIteration; i++){
            while(results.size() < this.numOfMaxResults){
                ...
                if(checkAvailability(newNode, Team.me().enemiesObservationSimulator.matrix, this.pointStatus)){
