...
public class LidarObservation extends LoopThread {
    ...
    public LidarObservation(RoboMaster roboMaster){
        ...
        switch (mode){
            ...
            case array -> {
                other_array = new PointState[8490][4890];
            }
            case both -> {
                others = new CopyOnWriteArrayList<>();
                other_array = new PointState[8490][4890];
    ...
    public CopyOnWriteArrayList<StatePoint> lidarPointCloudSimulate(float c_x, float c_y){
        ...
        for(float degree = 0;degree < 360; degree += precisionOfDegree){
            ...
            if(degree == 0 || degree == 180){
                for(y = 0;y <480; y++){
                    if(degree == 0){
                        if(Systems.pointSimulator.isPointNotEmpty(centre_x, centre_y + y, pointState)){
                ...
                for(x = 0;x <840; x++){
                    if(degree == 90){
                        if(Systems.pointSimulator.isPointNotEmpty(centre_x + x, centre_y, pointState)){
                ...
                for(y = 0;y <480; y++){
                    ...
                    if(Systems.pointSimulator.isPointNotEmpty(centre_x + offset_x, centre_y + offset_y, pointState)){
